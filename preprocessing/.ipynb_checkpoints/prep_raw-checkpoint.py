    ...
    def prep_video_frames(self):
        ...
        for utt_name in tqdm(self.utt_names):
            try:
                ...
                if vid_feature.dim() == 4:
                    # Adding an extra dimension if needed
                    vid_feature = vid_feature.unsqueeze(0)
                vid_features.append(vid_feature)
                imgs_lens.append(imgs_len)
            except Exception as e:
                print(f"Error processing {utt_name[:-1]}: {e}")
                # vid_features.append(torch.zeros(1, 3, 48, 48))
                vid_features.append(torch.zeros(1, 16, 3, 48, 48)) # 5D tensor = [batch, frames, channels, height, width]
                imgs_lens.append(1)

        ################################################################################################################
        # Noting the dimensions of each feature sample:
        # for i, feat in enumerate(vid_features):
        #     print(f"Feature {i} shape: {feat.shape}, dimensions: {feat.dim()}")
